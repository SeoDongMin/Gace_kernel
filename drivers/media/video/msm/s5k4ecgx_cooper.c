...
#ifdef COUNT_OF_FLASH
struct class *flash_class;
struct device *flash_dev;
static unsigned int flash_cnt = 0;

static void flashcnt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int retcnt=0;
	printk(KERN_DEBUG "flashecnt_show\n");
...
static DEVICE_ATTR(flashcnt, S_IRUGO |S_IRUSR, flashcnt_show, NULL);
#endif
...
{
	CAMDRV_DEBUG("s5k4ecgx_set_fps %d \n",value);
...
void s5k4ecgx_set_preview_size(int size)
{
	...
	{
		if (s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMCORDER)
		{
			CAMDRV_DEBUG("s5k4ecgx_set_preview_size : Camcorder");
...
*/
	switch(ctrl_info.mode)
	{
		...
		break;
		case EXT_CFG_SLOW_AE_AWB_CONTROL:
			CAMDRV_DEBUG("PCAM_VGA_CAMCORDER_SLOW_AE_AWB_CONTROL");
			S5K4ECGX_WRITE_LIST(s5k4ecgx_640_Camcorder);
		break;
		default :
				...
				struct msm_sensor_ctrl *s)
{
	...
	flash_dev = device_create(flash_class, NULL, 0, NULL, "camflash");

	if (device_create_file(flash_dev, &dev_attr_flashcnt) < 0)
		printk("[Camflash] Failed to create device file(%s)!\n", dev_attr_flashcnt.attr.name);
#endif
