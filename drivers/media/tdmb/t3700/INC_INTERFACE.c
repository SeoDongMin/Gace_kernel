...
INC_UINT8           g_IsFactoryMode = 0;
 ...
 #define STREAM_PARALLEL_ADDRESS			addr_TDMB_CS4_V//0x98000000 //-->EBI2CS4_BASE  //addr_TDMB_CS4_V
 #endif
/*********************************************************************************/
/*  RF Band Select						                                         */
/*																				 */
...
/*********************************************************************************/
PLL_MODE			m_ucPLL_Mode		= INPUT_CLOCK_24576KHZ;//INPUT_CLOCK_24576KHZ;
...
INC_UINT8 INC_EBI_READ_BURST(INC_UINT8 ucI2CID,  INC_UINT16 uiAddr, INC_UINT8* pData, INC_UINT16 nSize){    INC_UINT16 uiLoop, nIndex = 0, anLength[2], uiCMD, unTotal = 0;    INC_UINT16 uiNewAddr = (ucI2CID == TDMB_I2C_ID82) ? (uiAddr | 0x8000) : uiAddr;    if(nSize > INC_MPI_MAX_BUFF) return INC_ERROR;        memset((INC_INT8*)anLength, 0, sizeof(anLength)); /* parasoft-suppress MISRA2008-0_1_2_t "False Alarm - intended code" */    if(nSize > 0xfff) {        anLength[unTotal++] = 0xfff;        anLength[unTotal++] = nSize - 0xfff;    }    else anLength[unTotal++] = nSize;    local_irq_save(intlock);    for(uiLoop = 0; uiLoop < unTotal; uiLoop++){        uiCMD = INC_REGISTER_CTRL(SPI_MEMREAD_CMD) | (anLength[uiLoop] & 0xFFF);        *(volatile INC_UINT8*)STREAM_PARALLEL_ADDRESS = uiNewAddr >> 8;        *(volatile INC_UINT8*)STREAM_PARALLEL_ADDRESS = uiNewAddr & 0xff;        *(volatile INC_UINT8*)STREAM_PARALLEL_ADDRESS = uiCMD >> 8;        *(volatile INC_UINT8*)STREAM_PARALLEL_ADDRESS = uiCMD & 0xff;        for(nIndex = 0 ; nIndex < anLength[uiLoop]; nIndex++){            *pData++ = *(volatile INC_UINT8*)STREAM_PARALLEL_ADDRESS & 0xff;        }    }
   local_irq_restore(intlock);    return INC_SUCCESS;}
...
{
	memset(&g_stDmbInfo,	0, sizeof(ST_SUBCH_INFO)); /* parasoft-suppress MISRA2008-0_1_2_t "False Alarm - intended code" */
	memset(&g_stDabInfo,	0, sizeof(ST_SUBCH_INFO)); /* parasoft-suppress MISRA2008-0_1_2_t "False Alarm - intended code" */
	memset(&g_stDataInfo,	0, sizeof(ST_SUBCH_INFO)); /* parasoft-suppress MISRA2008-0_1_2_t "False Alarm - intended code" */
	memset(&g_stFIDCInfo,	0, sizeof(ST_SUBCH_INFO)); /* parasoft-suppress MISRA2008-0_1_2_t "False Alarm - intended code" */
	return INC_SUCCESS;
}
...
{
	INC_UINT16 unStatus;
    unStatus = INC_CMD_READ(ucI2CID, APB_MPI_BASE + 6);

    if(unStatus < INC_INTERRUPT_SIZE) return INC_ERROR;
