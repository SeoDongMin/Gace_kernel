...
#define PROGRESS_BAR_DRAW_ONCE 4
#else
#define PROGRESS_BAR_LEFT_POS	61
#define PROGRESS_BAR_RIGHT_POS	259
#define PROGRESS_BAR_START_Y	342
#define PROGRESS_BAR_WIDTH	1
#define PROGRESS_BAR_HEIGHT	6
#define PROGRESS_BAR_DRAW_ONCE 8
#endif
...
{
    0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00,
	0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00, 0x00, 0xC5, 0xFE, 0x00,
};

static unsigned char anycall_progress_bar_bg[] = 
{
...
{
    0x33,0x33,0x33,0x00,0x33,0x33,0x33,0x00,0x33,0x33,0x33,0x00,0x33,0x33,0x33,0x00,
    0x33,0x33,0x33,0x00,0x33,0x33,0x33,0x00,0x00,0x00
};

#endif

static void msm_fb_start_progress(struct fb_info *fb);
static void msm_fb_stop_progress(void);
...
#ifdef DISPLAY_BOOT_PROGRESS
static void msm_fb_update_framebuffer(struct fb_info *fb,
									int x, int y, void *buffer, 
									int src_width, int src_height)
{
	struct msm_fb_data_type *pdata = (struct msm_fb_data_type *)fb->par;
	...
	{
		dev_err(fb->device,"invalid destination coordinate or source size (%d, %d) (%d %d) \n", x, y, src_width, src_height);
		return;
	}	
...
{	
	...
	{
		msm_fb_update_framebuffer(fb,
			x_pos,
			PROGRESS_BAR_START_Y,
			(void*)anycall_progress_bar_bg,
			PROGRESS_BAR_WIDTH,
			PROGRESS_BAR_HEIGHT);
	}
#if !(defined(CONFIG_MACH_JUNO_SKT) || defined(CONFIG_MACH_JUNO_KT))
...
{	
	int i;	
	for(i = 0; i < PROGRESS_BAR_DRAW_ONCE; i++)	
	{
        if(progress_pos < PROGRESS_BAR_RIGHT_POS)
        {
    		msm_fb_update_framebuffer((struct fb_info *)data,
    			progress_pos++,
#if defined(CONFIG_MACH_JUNO_SKT) || defined(CONFIG_MACH_JUNO_KT)
                PROGRESS_BAR_START_Y,
#else
    			PROGRESS_BAR_START_Y+1,
#endif
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	/* Mark for GetLog */
#if defined(CONFIG_MACH_JUNO_SKT) || defined(CONFIG_MACH_JUNO_KT)
    frame_buf_mark.p_fb = fbram_phys - CONFIG_PHYS_OFFSET;
#else
...
#if defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_COOPER_BASE_KOR) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_LUCAS)
...
// 20100909 hongkuk.son for COOPER.rle ( booting logo )
	// if (!load_565rle_image_onfb( "CALLISTO.rle",0,0)) ;	/* Flip buffer */
     ...
     #if defined(CONFIG_MACH_JUNO_SKT) || defined(CONFIG_MACH_JUNO_KT)
     	if (!load_565rle_image_onfb( "VASTO.rle",0,0)) ;	/* Flip buffer */
     #else
     	if (!load_565rle_image_onfb( "COOPER_KOR.rle",0,0)) ;	/* Flip buffer */
     #endif
  #else
...
#ifdef DISPLAY_BOOT_PROGRESS
	msm_fb_start_progress(registered_fb[0]);
#endif
