...
msm_i2c_pwr_timer(unsigned long data)
{
	...
	if (dev->clk_state == 1)
	{
		printk("msm_i2c_pwr_timer : msm_i2c_pwr_mgmt\n");
...
msm_i2c_recover_bus_busy(struct msm_i2c_dev *dev, struct i2c_adapter *adap)
{
	...
	disable_irq(dev->irq);
	if (status & I2C_STATUS_RD_BUFFER_FULL) {
		dev_err(dev->dev, "Read buffer full, status %x, intf %x\n",
		...
		dev_err(dev->dev, "Still the bus master, status %x, intf %x\n",
			 status, readl(dev->base + I2C_INTERFACE_SELECT));
	...
	status = readl(dev->base + I2C_STATUS);
	if (!(status & I2C_STATUS_BUS_ACTIVE)) {
		dev_err(dev->dev, "Bus busy cleared after %d clock cycles, "
			 "status %x, intf %x\n",
	...
	dev_err(dev->dev, "------------ msm hw i2c dump start -------------\n");
	dev_err(dev->dev, "I2C_WRITE_DATA : %x\n", readl(dev->base + I2C_WRITE_DATA));
	dev_err(dev->dev, "I2C_CLK_CTL : %x\n", readl(dev->base + I2C_CLK_CTL));
	dev_err(dev->dev, "I2C_STATUS : %x\n", readl(dev->base + I2C_STATUS));
	dev_err(dev->dev, "I2C_READ_DATA : %x\n", readl(dev->base + I2C_READ_DATA));
	dev_err(dev->dev, "I2C_INTERFACE_SELECT : %x\n", readl(dev->base + I2C_INTERFACE_SELECT));
	dev_err(dev->dev, "I2C_NS_REG : %x\n", readl(dev->base + 0x0068));
	dev_err(dev->dev, "CLK_HALT_STATEA : %x\n", readl(dev->base + 0x0108));
	dev_err(dev->dev, "------------ msm hw i2c dump end -------------\n");
...
msm_i2c_remove(struct platform_device *pdev)
{
	...
	if (dev->clk_state != 0)
	{
		printk("msm_i2c_remove : msm_i2c_pwr_mgmt\n");
		msm_i2c_pwr_mgmt(dev, 0);
	}
...
static int msm_i2c_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (dev) {
		...
		if (dev->clk_state != 0)
		{
			printk("msm_i2c_suspend : msm_i2c_pwr_mgmt\n");
			msm_i2c_pwr_mgmt(dev, 0);
		}
	}
