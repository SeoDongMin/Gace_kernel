...
static int noise_gsm900  = 0;
#endif

//#if defined(CONFIG_MACH_GIO_KT)
//static int valid_noise = 0;
//extern int is_s5k5cagx_gio_power_on;
//#endif
...
#endif
#if defined(CONFIG_MACH_COOPER_SKT)
static ssize_t set_tsp_for_noise_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t set_tsp_for_noise_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
#endif
...
#endif
#if defined(CONFIG_MACH_COOPER_SKT)
static DEVICE_ATTR(set_tsp_for_noise, 0660 , set_tsp_for_noise_show, set_tsp_for_noise_store);
#endif
/* sys fs */
...
//    static int check_noise_count = 0;
//#endif
...
*/
	...
	for ( i= 0; i<MAX_USING_FINGER_NUM; i++ )
	{
		...
		//			if(fingerInfo[1].id ==0)
				...
				{
#if defined(CONFIG_MACH_COOPER_BASE_KOR)					
					if(ABS(fingerInfo[2].x,fingerInfo[0].x)> jump_limit_X)
#else
...
#if defined(CONFIG_MACH_COOPER_BASE_KOR)					
					else if(ABS(fingerInfo[2].y,fingerInfo[0].y)>jump_limit_Y)
#else                        
				...
				{
#if defined (CONFIG_MACH_COOPER_BASE_KOR) // force release ... // Move from Key to Screen, Key Release event do not occur.
                    int key;				
                    for(key = 0; key < MAX_KEYS; key++)
                    {
                        if(touchkey_status[key] == TK_STATUS_PRESS)
                        {
                            touchkey_status[key] = TK_STATUS_RELEASE;
                            // release event
#ifdef CONFIG_KERNEL_DEBUG_SEC                            
                            printk("[TSP] release keycode: %4d, keypress: %4d\n", touchkey_keycodes[key], 0);
#endif
                            input_report_key(ts_global->input_dev, touchkey_keycodes[key], 0);
                        }                            
                    }
#endif                    
					if(fingerInfo[i].status != -2) // force release
...
/*
	#if defined(CONFIG_MACH_GIO_KT)
	if( is_s5k5cagx_gio_power_on == 1 && fingerInfo[0].status == 1 && valid_noise == 0)
    {
        if(fingerInfo[0].x > 225 && fingerInfo[0].x < 240 && fingerInfo[0].y > 78 && fingerInfo[0].y < 95)
        {
            if (check_noise_count == 0) 
            {
                last_x = fingerInfo[0].x;
                last_y = fingerInfo[0].y;
                check_noise_count++;
            }
            else
            {
                if (fingerInfo[0].x < last_x-1 || fingerInfo[0].x > last_x+1 || fingerInfo[0].y < last_y-1 || fingerInfo[0].y > last_y+1)
                {
                    check_noise_count=0;    
                }
                else
        ...
        if (check_noise_count == 10)
        {
			printk("[TSP] %s vaild_noise is TRUE.\n", __func__);        
            valid_noise = 1;
            check_noise_count = 0;            
        }
...
#endif    
#if defined(CONFIG_MACH_COOPER_SKT)
    if (device_create_file(set_tsp_dev, &dev_attr_set_tsp_for_noise) < 0)    
        pr_err("Failed to create device file(%s)!\n", dev_attr_set_tsp_for_noise.attr.name);
#endif
...
/*
	...
	{
		if(buf[1] == 0x3 && buf[2] < 0x4)
		{
			firm_update( );
		}

		if(buf[1] == 0x23 && buf[2] < 0x4 )
        {
            firm_update( );
        }
	}
...
static void ts_resume_work_func(struct work_struct *work);
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	printk("[TSP] %s+\n", __func__ );
    //cancel_delayed_work_sync(&ts_resume_work);
...
static int synaptics_ts_resume(struct i2c_client *client)
{
    ...
    //	gpio_set_value( TSP_INT , 1 ); 
    	...
    	schedule_delayed_work(&ts_resume_work, 50); // 500ms
	}
    else
    {
		printk("[TSP] %s - TSP isn't present.\n", __func__ );
    }
...
#if defined(CONFIG_MACH_GIO_SKT) || defined (CONFIG_MACH_GIO_KT)
//	if ( HW_ver == 1  || HW_ver == 2 || HW_ver == 3 || HW_ver == 4 )
	if ( HW_ver == 3  || HW_ver == 0x23 )
#endif
	{
		/* for glass */
	...
	if( !(0 < buf_tmp[0] && buf_tmp[0] < 10) || !(0 < buf_tmp[1] && buf_tmp[1] < 10) )
	{
		//printk("[TSP] emergency firmware UPDATE\n");
		//firm_update();
...
{	
	...
	if(jump_limit_X == 70) 
		printk("show jump limit for input operation");
	else if(jump_limit_X == 180)
		printk("show jump limit for normal operation");
	else
		printk("show jump limit for Etc..");
...
{	
	...
	if(jump_limit_X == 70) 
		printk("set jump limit for input operation");
	else if(jump_limit_X == 180)
		printk("set jump limit for normal operation");
	else
		printk("set jump limit for Etc..");
...
{	
	...
	if(*buf == '1' && noise_gsm900 == 0) // GSM 900 CALL START 
	{
		wdog_val[0] = 1;
		ret = tsp_i2c_write( i2c_addr, wdog_val, sizeof(wdog_val));
        noise_gsm900 = 1;
	}
	else if(*buf == '0' && noise_gsm900 == 1) // GSM 900 CALL END
	{
		wdog_val[0] = 0;
		ret = tsp_i2c_write( i2c_addr, wdog_val, sizeof(wdog_val));
        noise_gsm900 = 0;
