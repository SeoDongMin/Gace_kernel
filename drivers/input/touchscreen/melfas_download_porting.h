...
extern int melfas_power_control(BOOLEAN OnOff);
#define MCSDL_VDD_SET_HIGH()             			melfas_power_control(1)
#define MCSDL_VDD_SET_LOW()              			melfas_power_control(0)
#else //MCSDL_USE_VDD_CONTROL
#define MCSDL_VDD_SET_HIGH()             			// Nothing
#define MCSDL_VDD_SET_LOW()              			// Nothing
#endif //MCSDL_USE_VDD_CONTROL
...
#define MCSDL_RESETB_SET_HIGH()             		gpio_set_value(TSP_INT, 1)
#define MCSDL_RESETB_SET_LOW()              		gpio_set_value(TSP_INT, 0)
#define MCSDL_RESETB_SET_OUTPUT(n)     		        gpio_direction_output(TSP_INT, n)
#define MCSDL_RESETB_SET_INPUT()            		gpio_direction_input(TSP_INT)
#else //MCSDL_USE_RESETB_CONTROL
#define MCSDL_RESETB_SET_HIGH()
#define MCSDL_RESETB_SET_LOW()
#define MCSDL_RESETB_SET_OUTPUT()
#define MCSDL_RESETB_SET_INPUT()
#endif //MCSDL_USE_RESETB_CONTROL
...
#define MCSDL_GPIO_SCL_SET_HIGH()					gpio_set_value(TSP_SCL, 1)
#define MCSDL_GPIO_SCL_SET_LOW()					gpio_set_value(TSP_SCL, 0)

#define MCSDL_GPIO_SDA_SET_HIGH()					gpio_set_value(TSP_SDA, 1)
#define MCSDL_GPIO_SDA_SET_LOW()					gpio_set_value(TSP_SDA, 0)

#define MCSDL_GPIO_SCL_SET_OUTPUT(n)				gpio_direction_output(TSP_SCL, n)
#define MCSDL_GPIO_SCL_SET_INPUT()					gpio_direction_input(TSP_SCL)

#define MCSDL_GPIO_SDA_SET_OUTPUT(n)				gpio_direction_output(TSP_SDA, n)
#define MCSDL_GPIO_SDA_SET_INPUT()					gpio_direction_input(TSP_SDA)

#define MCSDL_GPIO_SDA_IS_HIGH()					((gpio_get_value(TSP_SDA) > 0) ? 1 : 0)
