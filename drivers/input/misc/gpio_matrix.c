...
// kys0775_cooper 110114 remove CONFIG_MACH_COOPER_BASE_KOR
#if !defined(CONFIG_MACH_COOPER_BASE_KOR) && (defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_LUCAS))
unsigned int Volume_Up_irq = 0;
unsigned int Volume_Down_irq = 0;
#endif

// kys0775_cooper 110114 sleep current problem (side-key press)
#if defined(CONFIG_MACH_COOPER_BASE_KOR)

#define MAX_KEY_PRESS_COUNT   200

enum long_key_status{
  LONG_KEY_CHECK_INACTIVE,
  LONG_KEY_CHECK_ACTIVE,
  LONG_KEY_PRESSED,
};

#define GPIO_KEY_SCAN   35
#if defined(CONFIG_MACH_JUNO_SKT) || defined(CONFIG_MACH_JUNO_KT)
#define GPIO_KEY_SCAN1   34
#endif
#define GPIO_VOLUME_UP   42
#define GPIO_VOLUME_DOWN   41

unsigned int key_press_count = 0;
unsigned int long_key_state = LONG_KEY_CHECK_INACTIVE;
#endif
...
static irqreturn_t gpio_keypad_irq_handler(int irq_in, void *dev_id)
{
    ...
    if(long_key_state == LONG_KEY_PRESSED)
    {
        irq = gpio_to_irq(GPIO_VOLUME_UP);    			
