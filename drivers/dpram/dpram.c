...
static struct workqueue_struct *dpram_chg_wq;
static struct work_struct dpram_chg_detect_work;
static struct work_struct dpram_chg_state_work;
...
static void cmd_chg_detect_noti(void)
{
	...
	queue_work(dpram_chg_wq, &dpram_chg_detect_work);
}
...
static void cmd_chg_state_changed(void)
{
	...
	queue_work(dpram_chg_wq, &dpram_chg_state_work);
}
...
static void dpram_chg_detect_work_handler(struct work_struct * work)
{
	msleep(500);
	get_charger_type();
	get_charging_status();
	...
	(void)battery_restart();
}

static void dpram_chg_state_work_handler(struct work_struct * work)
{
	get_charging_status();
	msm_batt_check_event();
}
...
static int __init dpram_init(void)
{
	...
	INIT_WORK(&dpram_chg_detect_work, dpram_chg_detect_work_handler);
	INIT_WORK(&dpram_chg_state_work, dpram_chg_state_work_handler);
	dpram_chg_wq = create_singlethread_workqueue("dpram_chg_wq");
	if(dpram_chg_wq == NULL) {
		pr_err("Failed to create dpram_chg_wq!\n");
		return -1;
   }
error_return:
