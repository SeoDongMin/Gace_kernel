...
// cam i2c fast mode on arm11 - 400kbps
int msm_gpio_i2c_outb_scl_gpio2_sda_gpio3( unsigned char c)
{
	register int i;
	register unsigned gpio0_val;
	unsigned long irq_flags;
	
	spin_lock_irqsave(&(msm_gpio_chips[0].lock), irq_flags);
	
	writel(readl(MSM_GPIO_OE_0) | BIT(2), MSM_GPIO_OE_0);
	writel(readl(MSM_GPIO_OE_0) | BIT(3), MSM_GPIO_OE_0);

	gpio0_val = readl(MSM_GPIO_OUT_0); // optimization for gpio control  

	for (i = 7; i >= 0; i--) {
		if (((c >> i) & 1))
			gpio0_val |= BIT(3); // scl high
		else
			gpio0_val &= ~BIT(3); // sda low
		writel(gpio0_val , MSM_GPIO_OUT_0);
		writel(gpio0_val , MSM_GPIO_OUT_0); 
		writel(gpio0_val , MSM_GPIO_OUT_0); // for 900ns delay
		
		gpio0_val |= BIT(2); // scl high
		writel(gpio0_val , MSM_GPIO_OUT_0);
		writel(gpio0_val , MSM_GPIO_OUT_0); // for 900ns delay
		
		gpio0_val &= ~BIT(2); // scl low
		writel(gpio0_val , MSM_GPIO_OUT_0);
		writel(gpio0_val , MSM_GPIO_OUT_0); // for 600ns delay
	}

	spin_unlock_irqrestore(&(msm_gpio_chips[0].lock), irq_flags);
	return 1;
}
