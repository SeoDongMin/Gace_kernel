...
static int msm_vibrator_power(int on)
{
	...
	ret = vreg_set_level(vreg_msm_vibrator, OUT3300mV); //AVAD_yhan 20110214 3.1 -> 3.3V
...
static int __devinit msm_vibrator_probe(struct platform_device *pdev)
{
	...
	vibe_dev = device_create(vibe_class, NULL, 0, NULL, "vibrator");

	if (device_create_file(vibe_dev, &dev_attr_vibecnt) < 0)
		printk("[Vibrator] Failed to create device file(%s)!\n", dev_attr_vibecnt.attr.name);
#endif
